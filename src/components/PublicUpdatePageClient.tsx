...
export default function PublicUpdatePageClient({ projectId, versionId, initialVersion, initialProject, initialContent }: Props) {
  const [version, setVersion] = useState<Version | null>(initialVersion ?? null)
  const [project, setProject] = useState<Project | null>(initialProject ?? null)
  const [content, setContent] = useState<PageContent>(initialContent ?? { rows: [] })
  ...
  const fetchData = async () => {
    try {
      ...
      if (!versionRes.ok) {
        if (versionRes.status === 404) setError('Update not found')
        else if (versionRes.status === 403) setError('This update is not published yet')
        else setError('Failed to load update')
        setLoading(false)
        return
      }
  ...
  const gameHref = project?.slug ? `/g/${project.slug}` : project?.id ? `/game/${project.id}` : '/'
